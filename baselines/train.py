...
def train_esm_linear_mean(train_iterator, val_iterator, device, model, criterion, optimizer, epoch_num):
    ...
    for epoch in range(epoch_num):
        ...
        with torch.no_grad(): # evaluate validation loss here 
            ...
            for (inp, l) in val_iterator:
                ...
                out = model(inp)  # Forward prop without storing gradients

                loss = criterion(out, l) # Calculate validation loss 
                val_loss_epochs.append(loss.item())
